...
final class MapterhornOutputMetadata {
  ...
  static void complete(File output, Config config, CompletedFile completed)
      throws IOException {
    complete(output, config, completed, MapterhornFiles::atomicMove);
  }
  ...
  static String hex(byte[] bytes) {
    return MapterhornFiles.hex(bytes);
  }
  ...
  private static Plan inspect(File output, Config config, boolean overwriteExisting)
    ...
    if (!matches && metadata != null && metadata.state == State.PENDING
      && outputAttributes == null) {
      // A pending claim without output bytes guards nothing: it is the leftover of an
      // interrupted run under another archive version or resolution. Reclaim it, or a
      // republished archive would dead-lock every resumed 'all' run.
  ...
  private static Validation validateCompleted(Path outputPath,
    ...
    try (InputStream in = new BufferedInputStream(Channels.newInputStream(
      ...
      if (!ElevationCells.headerEquals(expected, actual)) {
        return Validation.invalid();
      }
    } catch (IOException e) {
      return Validation.invalid();
  ...
  private static void writeMetadata(Path metadataPath, Metadata metadata,
    ...
    try {
      try (FileChannel channel = FileChannel.open(temporary, StandardOpenOption.WRITE,
        ...
        channel.write(ByteBuffer.wrap(encoded.toByteArray()));
        // flush before the rename: a torn sidecar would read as foreign provenance
        // and block the cell until manually deleted
        channel.force(false);
      }
  ...
  private static MessageDigest sha256Digest() {
    return MapterhornFiles.sha256Digest();
  }
  ...
  private static void rejectSymbolicLink(Path path, BasicFileAttributes attributes)
      throws IOException {
    MapterhornFiles.rejectSymbolicLink(path, attributes, "Mapterhorn output");
  }
