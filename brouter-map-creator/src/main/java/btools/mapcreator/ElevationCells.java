...
public final class ElevationCells {
  ...
  /** The hgt format's void marker, remapped to {@link #NODATA} when reading hgt input. */
  public static final short HGT_VOID = -32767;

  /**
   * ElevationRasterCoder's border-skip sentinel: values at or below it inside an
   * encoded raster are sentinels, never elevations.
   */
  public static final short BORDER_SKIP = -32766;

  /** Smallest short value still distinguishable from {@link #NODATA}. */
  public static final short MIN_ENCODABLE = (short) (Short.MIN_VALUE + 1);

  private ElevationCells() {
  }

  /**
   * Field-for-field equality of two raster grid headers — the byte-compatibility
   * contract every .bef producer and validator must agree on.
   */
  public static boolean headerEquals(ElevationRaster a, ElevationRaster b) {
    return a.ncols == b.ncols && a.nrows == b.nrows && a.halfcol == b.halfcol
      && Double.doubleToLongBits(a.xllcorner) == Double.doubleToLongBits(b.xllcorner)
      && Double.doubleToLongBits(a.yllcorner) == Double.doubleToLongBits(b.yllcorner)
      && Double.doubleToLongBits(a.cellsize) == Double.doubleToLongBits(b.cellsize)
      && a.noDataValue == b.noDataValue;
  }

  /**
   * The file name of a cell under the scheme selected by
   * {@code PosUnifier.UseRasterRd5FileName} — the single place that choice is made.
   */
  public static String cellFileName(int lonStart, int latStart) {
    return PosUnifier.UseRasterRd5FileName
      ? rd5CellName(lonStart, latStart) : legacyCellName(lonStart, latStart);
  }

  /** Provenance label of the naming scheme behind {@link #cellFileName}. */
  public static String namingMode() {
    return PosUnifier.UseRasterRd5FileName ? "rd5" : "legacy";
  }
