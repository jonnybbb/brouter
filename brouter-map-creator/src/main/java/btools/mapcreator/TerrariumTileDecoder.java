...
public final class TerrariumTileDecoder {
  ...
  /**
   * Reusable per-thread buffer for {@link Raster#getPixels}: passing null would
   * allocate a fresh ~3-4 MB int[] for every decoded tile — terabytes of short-lived
   * garbage over a planet build. Capped at production tile sizes (512 px, up to 4
   * bands) so one oversized tile cannot pin a huge array to the thread.
   */
  private static final int MAX_SCRATCH_SAMPLES = 512 * 512 * 4;
  private static final ThreadLocal<int[]> SAMPLE_SCRATCH = new ThreadLocal<>();

  private TerrariumTileDecoder() {
  }

  /**
   * Disable ImageIO's temp-FILE-backed reads for bulk decoding: the default would
   * create/write/delete millions of files in java.io.tmpdir over a planet build.
   * This mutates JVM-global ImageIO state, so it is an explicit opt-in for bulk
   * converters rather than a side effect of loading this class.
   */
  public static void disableImageIoDiskCache() {
  ...
  private static Tile decodeImage(byte[] imageBytes, int expectedTileSize) throws IOException {
    ...
    if (isWebP(imageBytes)) {
      ...
      // Terrarium is only meaningful on a lossless image, and ImageIO would happily
      // decode a JPEG whose quantization noise becomes 256 m per red-channel count.
      throw new IOException("terrain tile is neither PNG nor WebP; refusing to decode"
        + " elevation from a possibly lossy format (" + imageBytes.length + " bytes)");
    }
  ...
  private static Tile decodePixels(BufferedImage image, int pixelCount) throws IOException {
    ...
    int[] samples;
    try {
      samples = raster.getPixels(0, 0, width, height, sampleScratch((int) sampleCount));
  ...
  private static int[] sampleScratch(int sampleCount) {
    if (sampleCount > MAX_SCRATCH_SAMPLES) {
      return new int[sampleCount]; // oversized tile: allocate, do not retain
    }
    int[] scratch = SAMPLE_SCRATCH.get();
    if (scratch == null || scratch.length < sampleCount) {
      scratch = new int[sampleCount];
      SAMPLE_SCRATCH.set(scratch);
    }
    return scratch;
  }
