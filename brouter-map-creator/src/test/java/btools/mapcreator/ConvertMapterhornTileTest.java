...
public class ConvertMapterhornTileTest {
  ...
  /**
   * A torn or corrupted cache entry (e.g. power loss between rename and data flush)
   * must not fail its cell forever: the entry is evicted and re-fetched from the
   * archive, and the healed cache serves later runs again.
   */
  @Test
  public void corruptCacheEntryIsEvictedAndRefetched() throws IOException {
    File cacheDir = tmp.newFolder("healing-cache");
    PmTilesTestArchive builder = archiveOf((gx, gy) -> 200.0 + (gx % 5) + (gy % 3));
    ...
    // corrupt every cached entry the way a torn write would: truncate to garbage
    java.util.List<java.nio.file.Path> entries;
    try (java.util.stream.Stream<java.nio.file.Path> walk = Files.walk(cacheDir.toPath())) {
      entries = walk.filter(p -> p.getFileName().toString().endsWith(".tile"))
        .collect(java.util.stream.Collectors.toList());
    }
    assertFalse("test premise: the cache must hold entries", entries.isEmpty());
    for (java.nio.file.Path entry : entries) {
      Files.write(entry, new byte[]{0, 1, 2});
    }

    ElevationRaster healed;
    try (PmTilesArchive archive = PmTilesArchive.open(builder.asByteSource());
         ConvertMapterhornTile converter = new ConvertMapterhornTile(archive, ZOOM, cacheDir, TILE)) {
      healed = converter.buildRaster(LON_START, LAT_START, ROW_LENGTH, null);
    }
    assertArrayEquals("conversion must self-heal over a corrupt cache",
      first.eval_array, healed.eval_array);

    // and the healed entries must be real tiles again, not the corrupt stubs
    for (java.nio.file.Path entry : entries) {
      assertTrue("entry must be rewritten: " + entry, Files.size(entry) > 3L);
    }
  }

  /**
   * Hard-crash-orphaned output temp files (finally-block cleanup never ran) must be
   * removed when the next run takes the output lock — nothing else ever deletes them.
   */
  @Test
  public void staleOutputTempFilesAreRemovedOnLockAcquisition() throws IOException {
    File outDir = tmp.newFolder("stale-temp-cleanup");
    File staleBef = new File(outDir, "srtm_38_03.bef.tmp-1234567890.tmp");
    File staleSidecar = new File(outDir,
      "srtm_38_03.bef" + MapterhornOutputMetadata.SUFFIX + ".tmp-42.tmp");
    File unrelated = new File(outDir, "keep-me.txt");
    File oldStyleName = new File(outDir, "cell.bef.tmp1");
    Files.write(staleBef.toPath(), new byte[]{1});
    Files.write(staleSidecar.toPath(), new byte[]{2});
    Files.write(unrelated.toPath(), new byte[]{3});
    Files.write(oldStyleName.toPath(), new byte[]{4});

    try (ConvertMapterhornTile.OutputDirectoryLock ignored =
           ConvertMapterhornTile.OutputDirectoryLock.acquire(outDir)) {
      assertFalse("stale .bef temp must be deleted", staleBef.exists());
      assertFalse("stale sidecar temp must be deleted", staleSidecar.exists());
      assertTrue("unrelated files must survive", unrelated.isFile());
      assertTrue("non-matching names must survive", oldStyleName.isFile());
    }
  }
  ...
  public void injectedHeaderMismatchFailsBeforeOutputPublication() throws IOException {
    ...
    try {
      ConvertMapterhornTile.writeVerified(raster, output, (expected, input, out) -> {
        ElevationRaster decoded = new ElevationRasterCoder().decodeRaster(input);
        decoded.cellsize *= 2.0;
        ConvertMapterhornTile.verifyReadBack(expected, decoded, out);
  ...
  public void atomicBefMoveFailurePreservesTargetAndPendingRecovery() throws IOException {
    ...
    try {
      ConvertMapterhornTile.writeVerified(raster, output,
        ConvertMapterhornTile::verifyReadBack, (from, to) -> {
  ...
  public void parseCellAcceptsBothSchemesAndRejectsGarbage() {
    ...
    // the prefix is case-insensitive, exactly like ElevationCells.cornerFromCellName
    assertArrayEquals(new int[]{5, 45}, ConvertMapterhornTile.parseCell("SRTM_38_03"));
   ...
   */
  @Test
  public void multiMemberGzipRunSlicesAndDecompressesEachTile() throws Exception {
    PmTilesTestArchive builder = new PmTilesTestArchive().zoomRange(ZOOM, ZOOM)
      .tileCompression(PmTilesArchive.COMPRESSION_GZIP)
    ...
    try (PmTilesArchive archive = PmTilesArchive.open(builder.asByteSource());
      ...
      java.util.Queue<Future<?>> decodes =
        new java.util.concurrent.ConcurrentLinkedQueue<>();
      converter.fetchRun(run, decoded, decodes);
      for (Future<?> decode; (decode = decodes.poll()) != null; ) {
        decode.get();
      }
  ...
  /**
   * PMTiles de-duplicates identical tiles into one location; a run holding the same
   * location twice must decode it once and share the immutable result across slots.
   */
  @Test
  public void deduplicatedLocationsAreDecodedOnceAndShared() throws Exception {
    PmTilesTestArchive builder = new PmTilesTestArchive().zoomRange(ZOOM, ZOOM)
      .put(ZOOM, 10, 11, TerrainTiles.constantPng(TILE, 500.0));

    try (PmTilesArchive archive = PmTilesArchive.open(builder.asByteSource());
         ConvertMapterhornTile converter = new ConvertMapterhornTile(archive, ZOOM, null, TILE)) {
      PmTilesArchive.TileLocation loc = archive.locateTile(ZOOM, 10, 11);
      java.util.List<ConvertMapterhornTile.MissedTile> run = java.util.Arrays.asList(
        new ConvertMapterhornTile.MissedTile(0, loc),
        new ConvertMapterhornTile.MissedTile(1, loc));
      java.util.concurrent.atomic.AtomicReferenceArray<TerrariumTileDecoder.Tile> decoded =
        new java.util.concurrent.atomic.AtomicReferenceArray<>(2);
      java.util.Queue<Future<?>> decodes =
        new java.util.concurrent.ConcurrentLinkedQueue<>();
      ...
      assertSame("identical locations must share one decoded tile",
        decoded.get(0), decoded.get(1));
      assertEquals(500.0, decoded.get(0).get(TILE / 2, TILE / 2), 1.0 / 256.0);
    }
  }
