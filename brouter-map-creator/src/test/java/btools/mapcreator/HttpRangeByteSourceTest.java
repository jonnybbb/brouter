...
public class HttpRangeByteSourceTest {
  ...
  public void servesRangeRequests() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/ok");
  ...
  public void exposesMetadataAfterSuccessfulRead() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/metadata");
  ...
  public void versionIdsDifferAcrossUrlsWithSameEtagAndLength() throws IOException {
    ...
    HttpRangeByteSource one =
      new HttpRangeByteSource(baseUrl + "/version-one");
    HttpRangeByteSource two =
      new HttpRangeByteSource(baseUrl + "/version-two");
  ...
  public void secondRequestSendsIfMatch() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/if-match");
  ...
  public void requestsIdentityContentEncoding() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/identity");
  ...
  public void changedEtagFails() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/changed-etag");
  ...
  public void wrongContentRangeFailsEvenWhenBodyLengthIsCorrect() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/wrong-range");
    try {
      src.read(4, 4);
  ...
  public void changedTotalLengthFails() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/changed-total");
  ...
  public void preconditionFailedIsNotRetried() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/precondition");
  ...
  public void rangeOverflowFailsBeforeOpeningConnection() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/overflow");
  ...
  public void successfulResponseThatIgnoresRangeIsNotRetried() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/range-ignored");
  ...
  public void notFoundIsNotRetried() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/gone");
    try {
      src.read(0, 4);
  ...
  public void rateLimitIsRetriedHonouringRetryAfter() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/limited");
  ...
  public void serverErrorIsRetried() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/flaky");
    assertArrayEquals("0123".getBytes(StandardCharsets.US_ASCII), src.read(0, 4));
    assertEquals(3, hits.get());
  }

  /**
   * 503 carries Retry-After exactly like 429 (RFC 9110) and is worth waiting out.
   */
  @Test
  public void serviceUnavailableIsRetriedHonouringRetryAfter() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/maintenance");
    assertArrayEquals("0123".getBytes(StandardCharsets.US_ASCII), src.read(0, 4));
    assertEquals(2, hits.get());
  }

  /**
   * 408 is a retryable timeout, not a deterministic 4xx failure.
   */
  @Test
  public void requestTimeoutIsRetried() throws IOException {
    ...
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + "/timeout");
    assertArrayEquals("0123".getBytes(StandardCharsets.US_ASCII), src.read(0, 4));
    assertEquals(2, hits.get());
  }

  /**
   * Retry-After accepts both RFC 9110 forms; garbage falls back to the default.
   */
  @Test
  public void retryAfterParsesSecondsAndHttpDates() {
    assertEquals(2000L, HttpRangeByteSource.parseRetryAfterMs("2"));
    assertEquals(0L, HttpRangeByteSource.parseRetryAfterMs("-1"));
    long dated = HttpRangeByteSource.parseRetryAfterMs(
      java.time.format.DateTimeFormatter.RFC_1123_DATE_TIME.format(
        java.time.ZonedDateTime.now(java.time.ZoneOffset.UTC).plusSeconds(30)));
    assertTrue("date-based delay must be in (0, 31s]: " + dated,
      dated > 0L && dated <= 31_000L);
    assertEquals(0L, HttpRangeByteSource.parseRetryAfterMs(
      "Mon, 01 Jan 2018 00:00:00 GMT")); // past dates clamp to zero
    assertEquals(1000L, HttpRangeByteSource.parseRetryAfterMs("soon"));
    assertEquals(1000L, HttpRangeByteSource.parseRetryAfterMs(null));
  }

  private void assertPermanentFailure(String path, String messagePart) throws IOException {
    HttpRangeByteSource src =
      new HttpRangeByteSource(baseUrl + path);
    try {
      src.read(0, 4);
  ...
  private static void assertSnapshotUnavailable(HttpRangeByteSource src)
      throws IOException {
    try {
      src.size();
