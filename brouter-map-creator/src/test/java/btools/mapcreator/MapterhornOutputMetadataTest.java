...
public class MapterhornOutputMetadataTest {
  ...
  /**
   * A leftover PENDING sidecar with a different config but no output bytes protects
   * nothing — an interrupted planet run followed by an archive republish (new
   * archiveId) must be reclaimable, not dead-locked until manual sidecar deletion.
   */
  @Test
  public void stalePendingSidecarWithoutOutputIsReclaimed() throws IOException {
    FilePair files = files("stale-pending.bef");
    assertEquals(MapterhornOutputMetadata.Decision.WRITE,
      MapterhornOutputMetadata.prepare(files.output, CONFIG_A, false));
    assertEquals("pending", load(files.metadata).getProperty("state"));
    assertFalse(files.output.exists());

    MapterhornOutputMetadata.Config republished = new MapterhornOutputMetadata.Config(
      "archive-b", 12, 3600, 512, "legacy", "full", 0, 45);
    assertEquals(MapterhornOutputMetadata.Decision.WRITE,
      MapterhornOutputMetadata.prepare(files.output, republished, false));
    assertEquals("archive-b", load(files.metadata).getProperty("archiveId"));
    assertEquals("pending", load(files.metadata).getProperty("state"));
  }

  /** The reclaim path must not weaken the guard when output bytes DO exist. */
  @Test
  public void mismatchingPendingSidecarWithOutputStillRefuses() throws IOException {
    FilePair files = files("pending-with-output.bef");
    assertEquals(MapterhornOutputMetadata.Decision.WRITE,
      MapterhornOutputMetadata.prepare(files.output, CONFIG_A, false));
    writeRaster(files.output, CONFIG_A, (byte) 5);

    MapterhornOutputMetadata.Config republished = new MapterhornOutputMetadata.Config(
      "archive-b", 12, 3600, 512, "legacy", "full", 0, 45);
    assertMismatch(files.output, republished);
    assertEquals("archive-a", load(files.metadata).getProperty("archiveId"));
  }

  @Test
  public void missingMalformedAndConflictingKeysAreMismatches() throws IOException {
    ...
    // a duplicate key resolves last-wins; a conflicting value must fail config equality
    Files.write(files.metadata.toPath(),
      (valid + "zoom=13\n").getBytes(StandardCharsets.ISO_8859_1));
    assertMismatch(files.output, CONFIG_A);
  }
  ...
  public void escapedDuplicateVersionStateAndDigestViolationsAreMismatches()
    ...
    String valid = new String(Files.readAllBytes(files.metadata.toPath()),
      StandardCharsets.ISO_8859_1);
    String[] invalid = {
      // escaped duplicate key decodes to a conflicting last-wins 'zoom'
      valid + "\\u007aoom=13\n",
