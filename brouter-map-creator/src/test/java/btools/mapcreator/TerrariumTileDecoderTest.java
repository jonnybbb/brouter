...
public class TerrariumTileDecoderTest {
  ...
  /**
   * Terrarium is only meaningful on lossless codecs. ImageIO would happily decode a
   * JPEG, whose quantization noise becomes 256 m per red-channel count.
   */
  @Test
  public void lossyJpegTilesAreRejected() throws IOException {
    BufferedImage image = new BufferedImage(8, 8, BufferedImage.TYPE_INT_RGB);
    ByteArrayOutputStream output = new ByteArrayOutputStream();
    assertTrue("no JPEG writer available", ImageIO.write(image, "jpeg", output));
    try {
      TerrariumTileDecoder.decode(output.toByteArray());
      fail("expected JPEG tile rejection");
    } catch (IOException expected) {
      assertTrue(expected.getMessage(),
        expected.getMessage().contains("neither PNG nor WebP"));
    }
  }
}
