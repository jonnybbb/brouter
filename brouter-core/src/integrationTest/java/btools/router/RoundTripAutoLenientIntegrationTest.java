...
public class RoundTripAutoLenientIntegrationTest {
  ...
  public void autoLenientlyAdoptsQualityFailedLoopWithWarning() {
    ...
    // The fixture is expected to force a hostile-surface QUALITY failure here;
    // if a future fixture/profile change makes fastbike pass cleanly, skip
    // rather than assert on a precondition that no longer holds.
    Assume.assumeNotNull("AUTO produced no best-effort track to adopt", track);
    String msg = infoMessage(track);
    Assume.assumeTrue("fixture no longer yields a lenient QUALITY warning here",
