...
public class RoutingEngineTest {
  ...
  public void fastRingRadiusScaleUsesConfidenceFilteredDirections() {
    double[] viable = RoutingEngine.fullCircleBearings(12);
  ...
  public void selectProbeDirectionKeepsFartherUsableMatch() {
    ...
    ProbeDirection selected = re.selectProbeDirection(90, matches, 0, 2,
      true, 2000, 10.0);
  ...
  // --- Optimized FAST placement characterization ---
  // Pins the behavior of the pieces moving into FastWaypointPlanner
  // (.agents/fast-waypoint-planner-spec.md) before the move. Island and
  // profile-hostile drops are NOT unit-characterizable pre-move: with a
  // dummy engine the island guard degrades to conservative-keep (findTrack
  // throws, caught) and cost evaluation is neutral (no wayDescription /
  // expctxWay). Both get scenario tests through the fake ops adapter.
  ...
  public void filterByProbeConfidenceDropsWeakWhenEnoughStrong() {
    ...
    double[] kept = RoutingEngine.filterByProbeConfidence(probe, 5);
  ...
  public void filterByProbeConfidenceKeepsWeakWhenFewStrong() {
    ...
    ProbeResult probe = probeWithStrongCounts(12, 8);
    double[] kept = RoutingEngine.filterByProbeConfidence(probe, 9);
  ...
  public void placeWaypointsFromProbeMatchesOrdersRingAndClosesLoop() {
    ...
    int placed = createDummyPlanner(re).placeWaypointsFromProbeMatches(wps, probe, -1, 7);
  ...
  public void placeWaypointsFromProbeMatchesCapsToTargetViaCount() {
    ...
    int placed = createDummyPlanner(re).placeWaypointsFromProbeMatches(wps, probe, -1, 5);
  ...
  public void placeWaypointsFromProbeMatchesIsDeterministic() {
    ...
    createDummyPlanner(re).placeWaypointsFromProbeMatches(first, probeWithDistinctMatches(viable), 45, 6);
    List<OsmNodeNamed> second = buildStartWaypointList();
    createDummyPlanner(re).placeWaypointsFromProbeMatches(second, probeWithDistinctMatches(viable), 45, 6);
  ...
  /** Planner wired to a dummy engine: island guard conservative, cost neutral. */
  private FastWaypointPlanner createDummyPlanner(RoutingEngine re) {
    return new FastWaypointPlanner(re.fastPlacementOps());
  }
