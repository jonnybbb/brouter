 ...
 */
public abstract class AbstractGreedyPlanner {
  ...
  protected final int subRouteCount;
  ...
  protected final LegRouter router;
  protected final EngineIO io;
  ...
  protected AbstractGreedyPlanner(RoundTripEngineOps engine, RoundTripCandidateProvider provider) {
  ...
  protected AbstractGreedyPlanner(LegRouter router, EngineIO io,
  ...
  private void populateResult(RoundTripResult result, OsmTrack track,
    ...
    List<MatchedWaypoint> travelStack = travelOrderedStack(waypointStack);
    result.setLoopWaypoints(buildLoopWaypoints(travelStack));
    result.setMatchedWaypoints(buildMatchedWaypoints(travelStack, startMwp));
  ...
  // ---- Orientation hooks -------------------------------------------------
  // The planner body is direction-agnostic: it commits legs between a "tail
  // head" (the via planned last) and candidates, keeps the committed legs in
  // TRAVEL order, and closes the loop between the tail head and the start.
  // The forward planner (GreedyRoundTripPlanner) routes tail head → candidate
  // and closes tail head → start; a closing-first planner routes candidate →
  // tail head, prepends, and closes start → tail head. Everything else —
  // candidate rounds, trials, detailing, gates, snapshots — is shared here.

  /** Session set-up once the start is matched: seed the tail head and via stack. False = fail (reason set). */
  protected abstract boolean initSession(GreedyPlanSession s, MatchedWaypoint startMwp);

  /** Route the leg between the tail head and a candidate, in travel direction. */
  protected abstract OsmTrack routeLeg(String name, MatchedWaypoint tailHead, MatchedWaypoint candidate,
                                       OsmTrack refTrack, long deadline);

  /** Detail-retrack a committed leg (see {@link #detailWithFallback}), in travel direction. */
  protected abstract OsmTrack detailLeg(String name, OsmTrack leg, MatchedWaypoint tailHead,
                                        MatchedWaypoint candidate, OsmTrack refTrack, long deadline);

  /** Route the closure between the tail head and the start (with the crossing variants). */
  protected abstract OsmTrack routeClosure(GreedyPlanSession s, List<OsmTrack> segments, OsmTrack ref,
                                           MatchedWaypoint tailHead, long deadline, int step);

  /** Detail-retrack the closure. */
  protected abstract OsmTrack detailClosure(String name, OsmTrack closure, GreedyPlanSession s,
                                            MatchedWaypoint tailHead, OsmTrack ref, long deadline);

  /** Last-resort closure past the plan deadline (no variants). */
  protected abstract OsmTrack routeForceClose(GreedyPlanSession s, MatchedWaypoint tailHead,
                                              OsmTrack ref, long deadline);

  /** Plain retrack of the force-close closure. */
  protected abstract OsmTrack retrackClosure(GreedyPlanSession s, OsmTrack closure, MatchedWaypoint tailHead);

  /** Commit a leg at the travel-order end the planner grows at. */
  protected abstract void commitLeg(List<OsmTrack> segments, OsmTrack leg);

  /** Undo the most recently committed leg. */
  protected abstract void uncommitLeg(List<OsmTrack> segments);

  /** Replace the most recently committed leg (raw → detailed). */
  protected abstract void replaceCommittedLeg(List<OsmTrack> segments, OsmTrack leg);

  /** Commit the closure into the committed legs (force-close path). */
  protected abstract void commitClosure(List<OsmTrack> segments, OsmTrack closure);

  /** A fresh list of the loop's legs in travel order, closure included. */
  protected abstract List<OsmTrack> loopLegs(List<OsmTrack> segments, OsmTrack closure);

  /** Join the committed prefix and a candidate leg into one tentative polyline (for crossing counts). */
  protected abstract void joinTentative(List<OsmPathElement> target, List<OsmPathElement> prefix,
                                        List<OsmPathElement> candidate);

  /** Planning-order via stack → travel order (start first). */
  protected abstract List<MatchedWaypoint> travelOrderedStack(List<MatchedWaypoint> stack);

  /** The bearing the direction preference judges a candidate by. */
  protected abstract double directionBearing(double bearingFromTailHead, int cpIlon, int cpIlat,
                                             GreedyPlanSession s);

  /** The step the direction fade is evaluated at (forward: the step itself). */
  protected abstract int directionStep(int step);

  /** The committed leg's node at the candidate end — the next tail head (forward: last node). */
  protected abstract OsmPathElement legFarEnd(OsmTrack leg);
  ...
  private ScoredCandidates generateAndScoreCandidates(GreedyPlanSession s, int step,
    ...
    if (s.dirPref != DirectionPreference.ANY && !candidates.isEmpty()) {
      double best = 180.0;
      for (RoundTripCandidateProvider.CandidatePoint cp : candidates) {
        double diff = CheapAngleMeter.getDifferenceFromDirection(s.dirPref.bearing,
          directionBearing(cp.bearing, cp.ilon, cp.ilat, s));
        if (diff < best) best = diff;
      }
      dirRef = best;
    }
    scorer.setDirectionReferenceOffset(dirRef);
    scorer.setDirectionStep(directionStep(step));
    ...
    for (RoundTripCandidateProvider.CandidatePoint cp : candidates) {
      ...
      cp.score = scorer.score(
        ...
        directionBearing(cp.bearing, cp.ilon, cp.ilat, s), s.dirPref,
        step, subRouteCount,
  ...
  private RoutedRound routeTopCandidates(GreedyPlanSession s, int step, int attempt,
    ...
    for (int r = 0; r < routeAttempts; r++) {
      ...
      if (subTrack == null) {
        subTrack = routeLeg("greedy-sub", fromMwp, toMwp, cachedRefTrack, stepDeadline);
      }
      if (subTrack == null && lastLegIslanded) {
        ...
        if (fromMoved || toMoved) {
          ...
          io.logInfo("greedy-sub: retrying islanded leg after pocket re-match ("
            ...
            + (toMoved ? "via" : "") + ")");
          subTrack = routeLeg("greedy-sub", fromMwp, toMwp, null, stepDeadline);
        }
      }
      s.candidatesRouted++;
      ...
      double routedScorerScore = scorer.score(
        ...
        s.totalDistance, estimatedReturn, desiredDistance,
        directionBearing(snappedBearing, snappedIlon, snappedIlat, s), s.dirPref,
        step, subRouteCount,
        actualVisitedRatio,
  ...
  private TrialOutcome tryCommitTrials(GreedyPlanSession s, int step, RoutedRound round,
    ...
    for (int trial = 0; trial < routedCandidates.size(); trial++) {
      ...
      commitLeg(segments, accepted.track);
      s.totalDistance += accepted.routeDistance;
      ...
      OsmPathElement lastNode = legFarEnd(accepted.track);
      ...
      if (returnChecked) {
        ...
        returnTrack = routeClosure(s, segments, returnRef, s.currentMwp, stepDeadline, step);
      ...
      replaceCommittedLeg(segments, detailedAccepted);
      ...
      OsmPathElement detailedEnd = legFarEnd(detailedAccepted);
      ...
      if (needDetail) {
        ...
        returnTrack = detailClosure("greedy-return-detail-fallback",
          returnTrack, s, s.currentMwp, detailedReturnRef, stepDeadline);
      }
      ...
      if (needDetail) {
        List<OsmTrack> loop = loopLegs(segments, returnTrack);
        finalTrack = mergeSegmentsDetoured(loop, null);
        reportSeamGaps(loop, null, result);
  ...
  private void recommitLeg(GreedyPlanSession s, DeferredCommit d, int currentIlon, int currentIlat) {
    ...
    commitLeg(s.segments, leg.track);
  ...
  private RoundTripResult finishPlan(GreedyPlanSession s) {
    ...
    if (!segments.isEmpty()) {
      ...
      OsmTrack returnTrack = routeForceClose(s, s.currentMwp, buildRefTrack(segments), forceCloseDeadline);
      s.returnChecksPerformed++;
      if (returnTrack != null && returnTrack.distance > 0) {
        returnTrack = retrackClosure(s, returnTrack, s.currentMwp);
        commitClosure(segments, returnTrack);
   ...
   */
  protected OsmTrack routeReturnWithVariants(List<OsmTrack> segments, OsmTrack returnRef,
  ...
  private OsmTrack detailAcceptedTrack(ScoredRoute accepted, MatchedWaypoint fromMwp,
                                       OsmTrack refTrack, long deadline) {
    return detailLeg("greedy-sub-detail-fallback",
   ...
   */
  protected OsmTrack detailWithFallback(String name, OsmTrack leg, MatchedWaypoint fromMwp,
   ...
   * </ul>
   */
  protected boolean detailFidelityTooLow(OsmTrack track) {
   ...
   */
  protected MatchedWaypoint matchCandidatePointProfileAware(int ilon, int ilat) {
    try {
  ...
  protected MatchedWaypoint matchPoint(int ilon, int ilat, String name) {
    try {
  ...
  private int countTentativeSelfIntersections(List<OsmPathElement> committedPrefixNodes,
    ...
    joinTentative(tentative.nodes, committedPrefixNodes, candidateSegment.nodes);
  ...
  protected void addVisitedEdges(OsmTrack track, VisitedEdgeStore edges,
  ...
  private Snapshot snapshotFallback(GreedyPlanSession s, OsmTrack track, OsmTrack returnTrack,
    ...
    snap.legTracks = loopLegs(s.segments, returnTrack);
    snap.error = error;
