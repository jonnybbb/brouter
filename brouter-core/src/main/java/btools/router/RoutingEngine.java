...
public class RoutingEngine extends Thread {
  ...
  void doGreedyRoundTrip(double searchRadius, double direction, RoundTripAlgorithm algo) {
    ...
    // coverage). Plain GREEDY plans build theirs lazily from the provider's
    // step-1 expansion inside the planner. Null = planner falls back to the
    // global-EMA estimate everywhere.
