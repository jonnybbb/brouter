...
```
java -Xmx2g -cp ... btools.mapcreator.ConvertMapterhornTile \
...
java -Xmx2g -cp '<map-creator runtime classpath>' btools.mapcreator.ConvertMapterhornTile \
...
reusing another build's cell. A stale *pending* claim without output bytes is
reclaimed, so an interrupted run followed by an archive republish stays resumable.

**Resume trust model.** Skipping an existing cell verifies the sidecar, the file's
size and identity, and its decoded raster header, and recomputes the full SHA-256
whenever the modification time differs from the recorded one. When size, identity
and modification time all match, the hash is deliberately *not* recomputed — reading
the whole output corpus on every resume would defeat resumability — so silent bit rot
that alters content while preserving size and mtime is not detected on resume. Delete
a suspect `.bef` (or its sidecar) to force reconversion.
...
Loading one source tile row is limited to 512 MiB of transient memory — while a
stripe is assembled its content exists twice (per-tile decode results plus the
assembled stripe), so the decoded stripe itself may be at most 256 MiB — and one
box-filter conversion is limited to about 8 billion visited source pixels. A request
over either limit fails before the large allocation or loop and tells the caller to
lower `-zoom` or narrow `-bbox`. A full 1-arcsecond cell still needs roughly
`-Xmx1500m` for its in-memory raster of about 648 MB; read-back verification streams
row by row against it, and at the default planet zoom a stripe is only ~60 MB.
